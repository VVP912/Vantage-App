...
export default function EdgeScreen({ onReplay }: Props) {
  ...
  const badge = (text: string, color: string, bg: string) => (
    <span style={{ fontSize: 14, padding: '2px 7px', borderRadius: 20, fontWeight: 500, color, background: bg }}>{text}</span>
  )

  const sourcePill = (text: string, isFree: boolean) => (
    <div style={{ fontSize: 14, color: isFree ? SOURCE_COLORS.free : SOURCE_COLORS.paid, marginTop: 6 }}>{isFree ? '✓ ' : '~'}{text}</div>
  )
  ...
  return (
    <div className="screen-light" style={{ padding: 18, maxWidth: 1200, margin: '0 auto' }}>
      {/* Header */}
      <div style={{ borderBottom: '0.5px solid var(--hairline)', paddingBottom: 14, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          <div style={{ width: 30, height: 30, background: 'var(--bg-void)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 500, color: 'var(--bull)', fontFamily: 'Courier New, monospace' }}>V</div>
          <div style={{ fontSize: 22, fontWeight: 500, color: 'var(--text-primary)' }}>VANTAGE</div>
        </div>
        <div style={{ fontSize: 16, color: 'var(--text-secondary)', marginTop: 3 }}>Alternative data intelligence. 7 real signals. Updated live.</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 8 }}>
          {['Yahoo Finance', 'Finnhub', 'SEC EDGAR', 'ApeWisdom', 'Google Maps', 'ESA Sentinel-2'].map(s => (
            <span key={s} style={{ fontSize: 14, padding: '2px 7px', borderRadius: 20, background: 'var(--bull-dim)', color: 'var(--bull)', fontWeight: 500 }}>{s}</span>
          ))}
        </div>
      </div>

      {/* Ticker selector */}
      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Select a stock</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6, marginBottom: 16 }}>
        {STOCKS.map((s, i) => (
          <button key={s.sym} onClick={() => setSelIdx(i)}
            ...
            <div style={{ fontSize: 16, fontWeight: 500, color: 'var(--text-primary)' }}>{s.sym}</div>
            <div style={{ fontSize: 14, color: 'var(--text-tertiary)', marginTop: 1 }}>{s.sector}</div>
            <div style={{ fontSize: 15, fontWeight: 500, marginTop: 5, color: s.altDataDir === 'bull' ? 'var(--bull)' : s.altDataDir === 'bear' ? 'var(--bear)' : 'var(--neutral)' }}>
      ...
      {quote?.price && (
        <div style={{ background: 'var(--bg-inset)', borderRadius: 8, padding: '10px 12px', marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 15, color: 'var(--text-tertiary)', marginBottom: 2 }}>{tk.sym} — live price</div>
            <div style={{ fontSize: 26, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'Courier New, monospace' }}>${quote.price?.toFixed(2)}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 18, fontWeight: 500, color: (quote.changePercent || 0) >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
              {(quote.changePercent || 0) >= 0 ? '+' : ''}{quote.changePercent?.toFixed(2)}%
            </div>
            <div style={{ fontSize: 14, color: 'var(--text-tertiary)', marginTop: 2 }}>Yahoo Finance · real-time</div>
          </div>
        </div>
      )}

      {loading ? (
        <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text-tertiary)', fontSize: 17 }}>Fetching 7 live data sources...</div>
      ) : (
        <>
          <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>7 alternative data signals</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 14 }}>
            ...
            {card(<>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Insider sentiment</div>
            ...
            {card(<>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>SEC Form 4 velocity</div>
            ...
            {card(<>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Reddit sentiment</div>
                {badge(reddit?.retailInterest || 'Unknown', 'var(--agentic)', 'var(--agentic-dim)')}
              </div>
              <div style={{ fontSize: 24, fontWeight: 500, color: 'var(--agentic)', fontFamily: 'Courier New, monospace' }}>
            ...
            {card(<>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>News sentiment</div>
                {badge(news?.bullishPercent != null ? (news.bullishPercent > 0.6 ? 'Bullish' : news?.bearishPercent && news.bearishPercent > 0.5 ? 'Bearish' : 'Mixed') : 'Loading', news?.bullishPercent != null && news.bullishPercent > 0.6 ? 'var(--bull)' : news?.bearishPercent && news.bearishPercent > 0.5 ? 'var(--bear)' : 'var(--neutral)', news?.bullishPercent != null && news.bullishPercent > 0.6 ? 'var(--bull-dim)' : news?.bearishPercent && news.bearishPercent > 0.5 ? 'var(--bear-dim)' : 'var(--neutral-dim)')}
              </div>
              <div style={{ fontSize: 24, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'Courier New, monospace' }}>
                {news?.bullishPercent != null ? `${Math.round(news.bullishPercent * 100)}% bull` : '—'}
              </div>
              <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 3, lineHeight: 1.4 }}>
                {news?.articleCount ? `${news.articleCount} articles · last 7 days` : 'Fetching live data...'}
              </div>
              {sourcePill(news?.sentimentSource === 'finnhub_premium' ? 'Finnhub premium · NLP sentiment' : 'Finnhub free tier · keyword heuristic', true)}
            </>, 'news')}

            {/* 7. Crypto macro (CoinGecko) — market-wide context, not stock-specific */}
            {card(<>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Crypto macro</div>
                {badge(
                  crypto?.available ? (crypto.macroScore > 0.15 ? 'Risk-on' : crypto.macroScore < -0.15 ? 'Risk-off' : 'Neutral') : 'Loading',
              ...
              <div style={{ fontSize: 24, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'Courier New, monospace' }}>
                {crypto?.available ? `${crypto.macroScore > 0 ? '+' : ''}${crypto.macroScore.toFixed(2)}` : '—'}
              </div>
              <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 3, lineHeight: 1.4 }}>
                {crypto?.available ? crypto.interpretation : 'Fetching live data...'}
              </div>
              {sourcePill('CoinGecko public API · BTC/ETH 24h momentum · free, no key', true)}
            </>, 'crypto')}
          ...
          {sat?.available && sat?.facilities && sat.facilities.filter(f => f.satelliteData).length > 0 && (
            <div style={{ background: 'var(--bg-inset)', borderRadius: 10, padding: 12, marginBottom: 14 }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
              ...
              {sat.facilities.filter(f => f.satelliteData).map((f, i) => (
                <div key={i} style={{ paddingBottom: 8, marginBottom: 8, borderBottom: i < sat.facilities!.length - 1 ? '0.5px solid var(--hairline)' : 'none' }}>
                  ...
                  <div style={{ fontSize: 14, color: f.satelliteData?.direction === 'elevated' ? 'var(--bull)' : f.satelliteData?.direction === 'reduced' ? 'var(--bear)' : 'var(--text-secondary)', marginTop: 2 }}>
                    Activity {f.satelliteData?.direction} · score {(f.satelliteData?.activityScore ?? 0) > 0 ? '+' : ''}{f.satelliteData?.activityScore?.toFixed(1)}
                  </div>
                  <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 2, lineHeight: 1.4 }}>{f.satelliteData?.interpretation}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ background: 'var(--bg-inset)', borderRadius: 10, padding: 12, marginBottom: 14 }}>
              ...
              {foot.locations.filter(l => l.currentBusyness !== null && l.currentBusyness !== undefined).map((loc, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 6, marginBottom: 6, borderBottom: '0.5px solid var(--hairline)' }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>{loc.name}</div>
                    <div style={{ fontSize: 14, color: 'var(--text-tertiary)' }}>{loc.type?.replace('_', ' ')}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 20, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'Courier New, monospace' }}>{loc.currentBusyness}/100</div>
                    <div style={{ fontSize: 14, color: 'var(--text-tertiary)' }}>{loc.signal}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Conviction call — big, unmissable, plain-language verdict.
              Pulls the recommended action directly from the deterministic
              risk model, paired with the agentic conviction level, so the
              headline number and the qualitative judgment agree visually. */}
          {conviction && prediction && (
            <div style={{
              background: conviction.convictionLevel === 'high' ? 'var(--bull-dim)' : conviction.convictionLevel === 'low' ? 'var(--bear-dim)' : 'var(--neutral-dim)',
              border: `2px solid ${conviction.convictionLevel === 'high' ? 'var(--bull)' : conviction.convictionLevel === 'low' ? 'var(--bear)' : 'var(--neutral)'}`,
              borderRadius: 14,
              padding: 20,
              marginBottom: 14,
              textAlign: 'center',
              ...
              <div style={{
                ...
                color: conviction.convictionLevel === 'high' ? 'var(--bull)' : conviction.convictionLevel === 'low' ? 'var(--bear)' : 'var(--neutral)',
                marginBottom: 8,
              }}>
                {conviction.convictionLevel.replace('_', ' ')} conviction · {conviction.agreeingSignalCount}/{conviction.totalLiveSignals} signals agree
              </div>
              <div style={{
                fontSize: 30, fontWeight: 700, lineHeight: 1.25,
                color: conviction.convictionLevel === 'high' ? 'var(--bull)' : conviction.convictionLevel === 'low' ? 'var(--bear)' : 'var(--neutral)',
              }}>
                {prediction.risk.recommendedAction}
              </div>
              <div style={{ fontSize: 16, color: 'var(--text-secondary)', marginTop: 10, lineHeight: 1.5 }}>
                Max position size: {prediction.risk.maxPositionSizePercent}% of portfolio
              </div>
            </div>
          )}
          ...
          <div style={{ background: 'var(--bg-panel)', border: '0.5px solid var(--hairline)', borderRadius: 12, padding: 12, marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Conviction agent
              </div>
              <span style={{ fontSize: 14, padding: '2px 7px', borderRadius: 20, background: 'var(--agentic-dim)', color: 'var(--agentic)', fontWeight: 500 }}>
                Agentic · tool call
              </span>
            </div>
            {convictionLoading && !conviction ? (
              <div style={{ fontSize: 16, color: 'var(--text-tertiary)' }}>Claude is weighing the 7 live signals against each other...</div>
            ) : conviction ? (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <span style={{
                    fontSize: 17, fontWeight: 500, padding: '3px 10px', borderRadius: 20,
                ...
                <div style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: 6 }}>{conviction.reasoning}</div>
              ...
              <div style={{ fontSize: 16, color: 'var(--text-tertiary)' }}>Conviction assessment unavailable this request.</div>
            )}
          </div>
          ...
          <div style={{ background: 'var(--bg-panel)', border: '0.5px solid var(--hairline)', borderRadius: 12, padding: 12, marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 14, padding: '2px 7px', borderRadius: 20, background: 'var(--macro-dim)', color: 'var(--macro)', fontWeight: 500 }}>
                Rule-based · weighted
              </span>
            </div>
            {!prediction ? (
              <div style={{ fontSize: 16, color: 'var(--text-tertiary)' }}>Computing weighted composite score...</div>
            ) : (
              <>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 26, fontWeight: 500, fontFamily: 'Courier New, monospace', color: prediction.prediction.predictedDirection === 'bullish' ? 'var(--bull)' : prediction.prediction.predictedDirection === 'bearish' ? 'var(--bear)' : 'var(--text-secondary)' }}>
                    {prediction.prediction.compositeScore > 0 ? '+' : ''}{prediction.prediction.compositeScore.toFixed(3)}
                  </span>
                  <span style={{ fontSize: 16, color: 'var(--text-secondary)', textTransform: 'capitalize' }}>
                ...
                <div style={{ marginBottom: 8 }}>
                  {prediction.prediction.signalBreakdown.map((s, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 14, color: 'var(--text-secondary)', width: 130, flexShrink: 0 }}>{s.label}</span>
                      ...
                      <span style={{ fontSize: 14, color: 'var(--text-tertiary)', width: 50, textAlign: 'right', flexShrink: 0 }}>w={s.weight}</span>
                    </div>
                  ))}
                </div>
                <div style={{ fontSize: 14, color: 'var(--text-tertiary)', lineHeight: 1.5, paddingTop: 6, borderTop: '0.5px solid var(--hairline)' }}>
          ...
          {prediction && (
            <div style={{ background: 'var(--bg-panel)', border: '0.5px solid var(--hairline)', borderRadius: 12, padding: 12, marginBottom: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <span style={{
                  fontSize: 14, padding: '2px 7px', borderRadius: 20, fontWeight: 500,
              ...
              <div style={{ fontSize: 24, fontWeight: 500, fontFamily: 'Courier New, monospace', color: 'var(--text-primary)', marginBottom: 6 }}>
                Max {prediction.risk.maxPositionSizePercent}% position
              </div>
              <div style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: 8 }}>{prediction.risk.rationale}</div>
              {prediction.risk.warnings.map((w, i) => (
                <div key={i} style={{ fontSize: 14, color: 'var(--bear)', lineHeight: 1.5, marginBottom: 4, paddingLeft: 10, borderLeft: '2px solid var(--bear)' }}>{w}</div>
              ))}
            </div>
          )}
          ...
          <div style={{ background: 'var(--bg-panel)', border: '0.5px solid var(--hairline)', borderRadius: 12, padding: 12, marginBottom: 14 }}>
            ...
            {onchainResult?.logged ? (
              <>
                <div style={{ fontSize: 16, color: 'var(--bull)', fontWeight: 500, marginBottom: 6 }}>Logged on-chain</div>
                <div style={{ fontSize: 14, color: 'var(--text-secondary)', wordBreak: 'break-all', marginBottom: 6 }}>Tx: {onchainResult.txHash}</div>
                {onchainResult.explorerUrl && (
                  <a href={onchainResult.explorerUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 15, color: 'var(--macro)' }}>
                    View on PolygonScan ↗
                  </a>
                )}
              </>
            ) : onchainResult && !onchainResult.logged ? (
              <div style={{ fontSize: 15, color: 'var(--text-tertiary)', lineHeight: 1.5 }}>{onchainResult.message}</div>
            ) : (
              <>
                ...
                <button
                  ...
                  style={{ padding: '8px 14px', background: conviction ? 'var(--bg-void)' : 'var(--text-tertiary)', color: 'var(--bg-panel)', border: 'none', borderRadius: 6, fontSize: 15, fontWeight: 500, cursor: conviction ? 'pointer' : 'not-allowed', fontFamily: 'Courier New, monospace' }}
                >
          ...
          <div style={{ background: 'var(--bg-void)', borderRadius: 12, padding: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 14, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 5 }}>
              AI synthesis — 8 signals vs analyst consensus
            </div>
            <div style={{ fontSize: 17, fontWeight: 500, color: 'var(--bg-panel)', lineHeight: 1.6, marginBottom: 8 }}>
              {tk.altDataDir === 'bear'
            ...
            <div style={{ fontSize: 15, color: 'var(--bull)', fontFamily: 'Courier New, monospace' }}>
              Satellite + foot traffic: most like institutional alt-data · Updated live
            </div>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.3)', marginTop: 3 }}>
          ...
          <div style={{ background: 'var(--bg-inset)', borderRadius: 12, padding: 12, marginBottom: 14 }}>
            <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Full analysis — synthesising all 7 live signals</div>
            <div style={{ fontSize: 16, color: 'var(--text-primary)', lineHeight: 1.8, fontFamily: 'Courier New, monospace', whiteSpace: 'pre-wrap' }}>
          ...
          {news?.articles && news.articles.length > 0 && (
            <div style={{ background: 'var(--bg-panel)', border: '0.5px solid var(--hairline)', borderRadius: 12, padding: 12, marginBottom: 14 }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Recent news · Finnhub</div>
              {news.articles.slice(0, 3).map((a, i) => (
                <div key={i} style={{ paddingBottom: 8, marginBottom: 8, borderBottom: i < 2 ? '0.5px solid var(--hairline)' : 'none' }}>
                  <div style={{ fontSize: 16, color: 'var(--text-primary)', fontWeight: 500, lineHeight: 1.4 }}>{a.headline}</div>
                  <div style={{ fontSize: 14, color: 'var(--text-tertiary)', marginTop: 3 }}>{a.source} · {a.datetime}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ background: 'var(--bg-inset)', borderRadius: 10, padding: 12, marginBottom: 14 }}>
            <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>All data sources — full transparency</div>
            {[
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, padding: '4px 0', borderBottom: i < 6 ? '0.5px solid var(--hairline)' : 'none' }}>
      ...
      <div style={{ background: 'var(--bg-inset)', borderRadius: 12, padding: 12, marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 500, color: 'var(--text-primary)' }}>$29<span style={{ fontSize: 17, fontWeight: 400, color: 'var(--text-tertiary)' }}>/month</span></div>
          <div style={{ fontSize: 15, color: 'var(--text-tertiary)', marginTop: 2 }}>8 signals · 50 stocks · Weekly updates · Full AI synthesis</div>
          <div style={{ fontSize: 14, color: 'var(--bull)', marginTop: 2 }}>Hedge funds pay $2-5M/year for the same data</div>
        </div>
        <button style={{ padding: '9px 18px', background: 'var(--bg-void)', color: 'var(--bg-panel)', border: 'none', borderRadius: 6, fontSize: 15, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'Courier New, monospace' }}>
          Start free trial
        </button>
      </div>

      <button onClick={onReplay} style={{ display: 'block', width: '100%', padding: 10, background: 'transparent', border: '0.5px solid var(--hairline)', borderRadius: 8, color: 'var(--text-tertiary)', fontSize: 15, cursor: 'pointer', letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: 'Courier New, monospace' }}>
        Play VANTAGE again ↗
      </button>
    </div>
  )
